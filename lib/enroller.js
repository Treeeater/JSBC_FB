...
var startTestFailedTimes = 0;			//used to count how many times startTest has failed in a row (due to weird timeout or last test failure).
...
var startTest = function(site){
	...
	try {
		...
		tabs[0].url = siteToTest;
		startTestFailedTimes = 0;
		...
		startTestFailedTimes++;
		if (startTestFailedTimes > 3) {
			startTestFailedTimes = 0;
			Utils.resetTab();
		}
...
exports.initTestSuiteWorker = function(worker){
	...
	/*testSuiteWorker.port.on("resetTabReady", function(){
		window.setTimeout(startTest.bind(window,siteToTest),1000);			//siteToTest is already prepared.
	});*/
...
exports.enroll = function(accounts, site, cb){
	...
	window.setTimeout(startTest.bind(window,site),2000);			//instead of calling setTimeout, we receive a signal from reset Tab and proceed.
}
